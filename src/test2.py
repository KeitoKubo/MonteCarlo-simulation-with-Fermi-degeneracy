...
def EMC(i):
    ...
    F_x_arr  = np.array([0.0, F_x])
    for F_index in range(len(F_x_arr)):
        cur_time = 0
        time_index = 1
        F[0] = F_x_arr[F_index]
        # main stream
    ...
    fig.tight_layout()
    fig.savefig('imgs/EMC_degeneracy/dist_function_F_0/' + "v_drift E_mean time response F_" + str(int(F_x * 1e-5)) + "e5/" + "EF_" + str(int(E_F * 1e3)) + "meV" + "_" + str(int(sim_time_index)))
