...
def EMC(index):
    ...
    for p_index in range(len(partition_arr)):
        for n_index in range(len(num_e_arr)):
            ...
            fig.tight_layout()
            ani.save('imgs/EMC_degeneracy/dist_function_F_0/'  + "p_" + str(partition) + "_" + str(int(num_e * 1e-5)) + "e5/" + ani_name, writer='pillow')
            fig.savefig('imgs/EMC_degeneracy/dist_function_F_0/'  + "p_" + str(partition) + "_" + str(int(num_e * 1e-5)) + "e5/" + "EF_" + str(int(E_F * 1e3)) + "meV" + "_" + str(int(sim_time_index)))
            plt.clf()
